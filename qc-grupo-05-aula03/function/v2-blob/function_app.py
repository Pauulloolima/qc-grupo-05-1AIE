...
def calcular_frete(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not cep:
        return func.HttpResponse(
            json.dumps({"erro": "Parâmetro 'cep' é obrigatório."}),
            mimetype="application/json",
            status_code=400
        )
    
    # Lógica simulada de frete: CEPs começando com '0' (como São Paulo) são mais baratos
    valor_frete = 15.00 if cep.startswith('0') else 35.00
    prazo_dias = 2 if cep.startswith('0') else 7
    
    resposta = {
        "cep_destino": cep,
        "valor_frete": valor_frete,
        "prazo_dias": prazo_dias,
        "transportadora": "QC Logística"
    }
